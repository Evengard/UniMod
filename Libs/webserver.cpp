...
bool httpAuthCurrentlyActive=false;
queue<char*> authQueue;
HANDLE authGetThreadH;
...
unsigned __stdcall httpAuth(void *Data1)
{
	char* Data=(char*)Data1;
	struct in_addr IP2;
	IP2.S_un.S_addr=(long)*currentIP;
	const char* IP=inet_ntoa(IP2);
	char Port2[6];
	itoa((int)((unsigned __int16)*currentPort), Port2, 10);
	const char* PathWithoutParams="http://servers.noxworld.ru/index.php";//?login=&pass=&action=login&ip=&port=
	char* Src1 = NULL;
	int strlenLogin=strlen(authorisedLogins[(byte)Data[0]]);
	int strlenPassword=strlen(&Data[1]);
	int strlenIP=strlen(IP);
	int strlenPath=strlen(PathWithoutParams);
	int strlenSum=strlenPath+strlenLogin+strlenPassword+strlenIP+45;
	Src1 = new char[strlenSum];
	strcpy(Src1, PathWithoutParams);
	strcat(Src1, "?action=login&ip=");
	strcat(Src1, IP);
	strcat(Src1, "&port=");
	strcat(Src1, Port2);
	strcat(Src1, "&login=");
	strcat(Src1, authorisedLogins[(byte)Data[0]]);
	strcat(Src1, "&pass=");
	strcat(Src1, &Data[1]);
	const char *Host=0,*Port=0,*Href=0,*P;
	const char* Src=Src1;
	if (0==strncmp(Src,"http://",7))
		Src+=7;
	Host=Src;
...
void AuthProcess()
{
	...
	{
		char* playerIdx = authQueue.front();
		authQueue.pop();
		if(currentAuth==true)
			authorisedState[(byte)playerIdx[0]]++;
		else
		{
			authorisedState[(byte)playerIdx[0]]-=2;
			delete [] authorisedLogins[(byte)playerIdx[0]];
			authorisedLogins[(byte)playerIdx[0]]="";
		}
		httpAuthCurrentlyActive=false;
		currentAuth==false;
	}
	if(authQueue.empty()==false && httpAuthCurrentlyActive==false)
	{
		char* data = authQueue.front();
		currentAuth==false;
		authGetThreadH = (HANDLE)_beginthreadex(NULL, 0, &httpAuth, (void*)data, 0, NULL);
		httpAuthCurrentlyActive=true;
	}
	return;
}

void authAddToList(char* Data)
{
	authQueue.push(Data);
}